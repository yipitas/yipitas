...
import {
  ...
  Upload, FileSpreadsheet, CheckCircle, Image,
} from 'lucide-react'
    ...
    reader.onload = (ev) => {
      ...
      const normalized = rows.map(r => {
        ...
        return {
          ...
          nombre: String(find(['nombre', 'name', 'producto', 'descripcion', 'descripción']) || '').trim(),
          categoria: String(find(['categoria', 'categoría', 'category', 'rubro']) || 'Otro').trim() || 'Otro',
          talla: String(find(['talla', 'talle', 'size', 'tallle']) || 'Único').trim() || 'Único',
          color: String(find(['color', 'colour']) || '').trim(),
          precio: Number(find(['precio_venta', 'precioventa', 'precio', 'price', 'pvp']) || 0),
          stock: Number(find(['stock', 'cantidad', 'qty', 'quantity']) || 0),
        }
      }).filter(r => r.nombre)
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div
                  ...
                  className="border-2 border-dashed border-gray-200 rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer hover:border-primary-300 hover:bg-primary-50 transition-colors min-h-[120px]"
                >
                  {imgPreview ? (
                ...
                {imgPreview && (
                  <button
                    type="button"
                    onClick={() => { setImgPreview(null); setForm(f => ({ ...f, foto_url: '' })) }}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving || uploadingImg} className="flex-1 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-300 text-white rounded-lg text-sm font-medium transition-colors">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-3xl shadow-xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={cerrarImport} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {importResult && (
              <div className="mx-6 mt-4 bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
                ...
                <p className="text-sm text-green-800">
                  Importación completada: <strong>{importResult.creados} creados</strong> y <strong>{importResult.actualizados} actualizados</strong>.
                </p>
              </div>
            )}
            ...
            {!importResult && (
              <div className="overflow-auto flex-1 px-6 py-4">
                <p className="text-xs text-gray-500 mb-3">Revisá los datos antes de confirmar. Si un producto tiene código y ya existe, se actualizará.</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 text-gray-500 text-xs">
                      <th className="text-left px-3 py-2 font-medium rounded-tl-lg">Código</th>
                      ...
                      <th className="text-right px-3 py-2 font-medium rounded-tr-lg">Stock</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {excelRows.map((r, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2">
                          <span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded text-xs">{r.talla}</span>
                        </td>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
              <button onClick={cerrarImport} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
              ...
              {!importResult && (
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-300 text-white rounded-lg text-sm font-medium transition-colors"
                >
                  {importing ? 'Importando...' : `Confirmar importación (${excelRows.length})`}
                </button>
              )}
            </div>
          </div>
